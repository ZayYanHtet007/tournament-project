...
<div class="main-content">
    <div class="main-content-container">
        ...
        <div class="pagination">
            <?php if ($page > 1): ?>
                <a href="?page=<?= $page - 1 ?>&search=<?= urlencode($searchTerm) ?>&start_from=<?= urlencode($startDateFrom) ?>&start_to=<?= urlencode($startDateTo) ?>&status=<?= urlencode($status) ?>" class="pg-link"><i class="fas fa-chevron-left"></i> PREV</a>
            <?php endif; ?>
            ...
            for ($i = $start_loop; $i <= $end_loop; $i++): ?>
                <a href="?page=<?= $i ?>&search=<?= urlencode($searchTerm) ?>&start_from=<?= urlencode($startDateFrom) ?>&start_to=<?= urlencode($startDateTo) ?>&status=<?= urlencode($status) ?>" class="pg-link <?= ($i == $page) ? 'active' : '' ?>"><?= $i ?></a>
            <?php endfor; ?>

            <?php if ($page < $total_pages): ?>
                <a href="?page=<?= $page + 1 ?>&search=<?= urlencode($searchTerm) ?>&start_from=<?= urlencode($startDateFrom) ?>&start_to=<?= urlencode($startDateTo) ?>&status=<?= urlencode($status) ?>" class="pg-link">NEXT <i class="fas fa-chevron-right"></i></a>
            <?php endif; ?>
        </div>
    </div>
</div>
